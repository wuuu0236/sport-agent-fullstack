...
class CoachAgent(BaseAgent):
    ...
    # 伤痛 / 康复关键词。
    # 重要：意图分类（_classify）与提示词构建（_build_prompt）必须共用这一份，
    # 否则会出现「判定为伤痛 → 却让模型排『膝盖疼训练日』」的自相矛盾
    # —— 那与同时注入的 posture_relief SOP（定位诱因→康复动作→就医红线）正面冲突。
    # 2026-09-07 修：两处判断合并为 _is_pain()。
    _PAIN_WORDS = ("疼", "痛", "受伤", "拉伤", "扭伤", "康复",
                   "不适", "酸", "肿", "炎症", "筋膜", "体态",
                   "圆肩", "劳损")

    @classmethod
    def _is_pain(cls, msg: str) -> bool:
        """是否伤痛 / 康复类消息。分类与提示词构建共用同一个判断。"""
        return any(k in msg for k in cls._PAIN_WORDS)
    ...
    def _classify(self, msg):
        ...
        if self._is_pain(msg):
            return "chat"

        # 概念解释 / 技术讲解类：问「是什么 / 什么意思 / 解释 / 区别」，
        # 或要「要点 / 技巧 / 怎么做」等讲动作要领的，一律交给 LLM 对话解答，
        # 不要误判成「分析数据」而硬塞训练记录。
        # 「要点/技巧/怎么做」这组是 2026-09-13 补的：它们此前不在任何白名单里，
        # 会掉进末尾「短消息 + 抠出动作名 → 力量趋势分析」的兜底分支，答非所问——
        # 「总结一下卧推要点」被抠出动作名「总结」、回「『总结』还没记录过训练数据」，
        # 「卧推要点」「深蹲技巧」「引体向上怎么做」同理。对照：带「发力」的
        # 「总结一下卧推的发力要点」因命中下方 training-advice 白名单而侥幸走对，
        # 说明漏的是这组技术问法词，不是兜底逻辑本身（不收紧兜底，避免误伤既有行为）。
        is_concept_q = any(k in msg for k in (
            "是什么", "什么意思", "意思是", "解释", "怎么理解", "含义",
            "指什么", "为什么", "区别", "如何定义", "科普", "讲讲", "说明", "怎么算",
            "要点", "要诀", "技巧", "要领", "诀窍", "注意事项", "注意什么",
            "怎么做", "怎么用", "如何做", "如何用", "怎么选", "讲究", "细节",
        ))
    ...
    def _build_prompt(self, msg):
        ...
        focus_hint = ""
        if self._is_pain(msg):
            # 伤痛 / 康复类：绝不能再引导模型去排「XX 训练日」——
            # 那会和 posture_relief 的 SOP 打架（一个让练、一个让先查诱因并给就医红线）。
            # 这里改成与 SOP 同向的康复引导，而不是把 focus 直接拼进去。
            focus_hint = (
                "\n用户提到疼痛或不适，不要排训练计划、也不要给训练日动作清单。"
                "按康复思路走：先定位部位与可能诱因（跑量骤增 / 肌力失衡 / 动作模式错误 / 装备），"
                "再给 3-4 个居家康复动作（明确组数次数）、说明先减量观察 2-3 天；"
                "命中就医红线（剧烈肿痛 / 放射性麻木 / 休息后无缓解 / 红肿发热 / 无法负重）"
                "要明确建议停训就医，并声明不替代医学诊断。")
        elif focus:
